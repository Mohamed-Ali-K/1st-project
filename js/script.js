...
$(function() {
    ...
    $("input[name='user_email']").blur(function() {
        if ($(this).val() == "") {
            ...
            $.ajax({
                ...
                type: "POST",
                success: function showemail(data) {
                    $("#loaderIcon2").hide();
                    if (data) {
                        $("#form-message2").html("<span><b style=\"color: Green\"> E-mail Available.</b><img src=\"tick.png\"></span>");
                    } else {
                        $("#form-message2").html("<span><b style=\"color: red\"> E-mail Not Available.</b><img src=\"cross.png\"></span>");
                    }
                },
                error: function() {}
            });
    ...
    $("input[name='user_uid']").blur(function() {
        if ($(this).val() == "") {
            ...
            $.ajax({
                ...
                type: "POST",
                success: function showuers(data) {
                    $("#loaderIcon").hide();
                    if (data) {
                        $("#form-message").html("<span><b style=\"color: Green\"> Username Available.</b><img src=\"tick.png\"></span>");
                    } else {
                        $("#form-message").html("<span><b style=\"color: red\"> Username Not Available.</b><img src=\"cross.png\"></span>");
                    }
    ...
    $("input[name='ruser_password']").blur(function() {
        if ($(this).val() == "") {
            $(this).css('border', 'solid 2px red');
    ...
    $("button[name='singup']").click(function() {
        var user_frist = $("#user_frist").val();
        var user_last = $("#user_last").val();
        var user_email = $("#user_email").val();
        var user_uid = $("#user_uid").val();
        var user_password = $("#user_password").val();
        var ruser_password = $("#ruser_password").val();
        /*
        console.log(user_frist);
        console.log(user_last);
        console.log(user_email);
        console.log(user_uid);
        console.log(user_password);
        console.log(ruser_password);
        */
        ...
        if (ruser_password == '') {
            event.preventDefault();
            $("label[for='ruser_password']").replaceWith(" <label style=\"bold\" for=\"user_password\">Password: <b style=\"color: red\">Password requre !</b></label>");
        }
        if (!(user_password).match(ruser_password)) {
            event.preventDefault();
            $("label[for='ruser_password']").replaceWith(" <label style=\"bold\" for=\"ruser_password\">Repet Password: <b style=\"color: red\">Password Don't match </b></label>");
        }
        if ((user_password.length) < 8) {
            event.preventDefault();
            $("label[for='user_password']").replaceWith(" <label style=\"bold\" for=\"user_password\">Password: <b style=\"color: red\">Password must be > 8 caracteur !</b></label>");
        }
    ...
    $("button[name='submit0']").click(function() {
